...
const emptyTicket = { name: "", price: "", quota: "", description: "" };

// ── Komponen form tiket terpisah biar bisa pakai useState ──
function TicketForm({ onAdd, loading }: { onAdd: (t: any) => void; loading: boolean }) {
  const [t, setT] = useState(emptyTicket);
  return (
    <div style={{ background: "#FAFAFA", borderRadius: 14, padding: 14, border: "1.5px dashed #DDD6FE", marginTop: 8 }}>
      <div style={{ fontSize: 12, fontWeight: 800, color: "#7C3AED", marginBottom: 10 }}>+ Tambah Tiket Baru</div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
        <div>
          <label style={{ fontSize: 10, fontWeight: 800, color: "#9CA3AF", display: "block", marginBottom: 4, textTransform: "uppercase" }}>Nama Tiket</label>
          <input style={{ width: "100%", padding: "9px 12px", border: "1.5px solid #DDD6FE", borderRadius: 10, fontSize: 13, fontWeight: 600, outline: "none", boxSizing: "border-box" as any, fontFamily: "'Nunito',sans-serif", background: "white" }}
            placeholder="Festival / VIP / VVIP" value={t.name} onChange={e => setT({ ...t, name: e.target.value })} />
        </div>
        <div>
          <label style={{ fontSize: 10, fontWeight: 800, color: "#9CA3AF", display: "block", marginBottom: 4, textTransform: "uppercase" }}>Harga (Rp)</label>
          <input style={{ width: "100%", padding: "9px 12px", border: "1.5px solid #DDD6FE", borderRadius: 10, fontSize: 13, fontWeight: 600, outline: "none", boxSizing: "border-box" as any, fontFamily: "'Nunito',sans-serif", background: "white" }}
            placeholder="750000" type="number" value={t.price} onChange={e => setT({ ...t, price: e.target.value })} />
        </div>
        <div>
          <label style={{ fontSize: 10, fontWeight: 800, color: "#9CA3AF", display: "block", marginBottom: 4, textTransform: "uppercase" }}>Kuota</label>
          <input style={{ width: "100%", padding: "9px 12px", border: "1.5px solid #DDD6FE", borderRadius: 10, fontSize: 13, fontWeight: 600, outline: "none", boxSizing: "border-box" as any, fontFamily: "'Nunito',sans-serif", background: "white" }}
            placeholder="1000" type="number" value={t.quota} onChange={e => setT({ ...t, quota: e.target.value })} />
        </div>
        <div>
          <label style={{ fontSize: 10, fontWeight: 800, color: "#9CA3AF", display: "block", marginBottom: 4, textTransform: "uppercase" }}>Deskripsi</label>
          <input style={{ width: "100%", padding: "9px 12px", border: "1.5px solid #DDD6FE", borderRadius: 10, fontSize: 13, fontWeight: 600, outline: "none", boxSizing: "border-box" as any, fontFamily: "'Nunito',sans-serif", background: "white" }}
            placeholder="Akses area umum" value={t.description} onChange={e => setT({ ...t, description: e.target.value })} />
        </div>
      </div>
      <button onClick={() => { onAdd(t); setT(emptyTicket); }} disabled={loading}
        style={{ width: "100%", padding: "10px 0", background: "linear-gradient(135deg,#7C3AED,#9333EA)", color: "white", border: "none", borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: loading ? "not-allowed" : "pointer", fontFamily: "'Nunito',sans-serif", opacity: loading ? 0.6 : 1 }}>
        {loading ? "⏳ Menyimpan..." : "✅ Tambah Tiket Ini"}
      </button>
    </div>
  );
}
...
export default function AdminDashboard() {
  ...
  const fetchTickets = async (eventId: number) => {
    try {
      const res = await axios.get(`/api/events/${eventId}/tickets`);
  ...
  const handleSaveEvent = async () => {
    ...
    try {
      ...
      setShowModal(false); setForm(emptyForm); setEditEvent(null); setTickets([]);
      fetchEvents();
    } catch (err: any) {
  ...
  const handleAddTicket = async (ticket: any) => {
    if (!editEvent) return;
    if (!ticket.name || !ticket.price || !ticket.quota) { toast.error("Nama, harga, dan kuota wajib diisi!"); return; }
    ...
    try {
      ...
      toast.success(`Tiket ${ticket.name} berhasil ditambah! 🎫`);
      fetchTickets(editEvent.id);
    } catch (err: any) {
      toast.error(err.response?.data?.message || "Gagal tambah tiket!");
    } finally { setTicketLoading(false); }
  };

  const handleDeleteTicket = async (ticketId: number) => {
    try {
      await axios.delete(`/api/admin/tickets/${ticketId}`, { headers: { Authorization: `Bearer ${token}` } });
      toast.success("Tiket dihapus!");
      if (editEvent) fetchTickets(editEvent.id);
    } catch { toast.error("Gagal hapus tiket!"); }
  };
  ...
  const downloadCSV = () => {
    ...
    const rows = bookings.map(b => [b.booking_code, b.user.name, b.user.email, b.event.title, b.ticket.name, b.quantity, toNum(b.total_price), b.status, formatDate(b.created_at)]);
    const csv = [headers, ...rows].map(r => r.join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `laporan-tiketin-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
    URL.revokeObjectURL(url);
    toast.success("Laporan berhasil didownload! 📊");
  };

  const downloadSummary = () => {
    const lines = ["===== LAPORAN TIKETIN =====", `Tanggal: ${new Date().toLocaleDateString("id-ID")}`, "", `Total Revenue: ${formatPrice(totalRevenue)}`, `Total Booking: ${bookings.length}`, `Booking Berhasil: ${paidBookings}`, `Booking Dibatal: ${bookings.length - paidBookings}`, `Total Event Aktif: ${events.length}`, `Konversi: ${bookings.length > 0 ? Math.round((paidBookings / bookings.length) * 100) : 0}%`, "", "===== DETAIL BOOKING =====", ...bookings.map(b => `${b.booking_code} | ${b.user.name} | ${b.event.title} | ${b.ticket.name} | ${formatPrice(toNum(b.total_price))} | ${b.status}`)];
    const blob = new Blob([lines.join("\n")], { type: "text/plain;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `ringkasan-tiketin-${new Date().toISOString().slice(0, 10)}.txt`; a.click();
    URL.revokeObjectURL(url);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F5F3FF", fontFamily: "'Nunito', sans-serif", display: "flex" }}>
      ...
      {/* SIDEBAR */}
      <div className={sidebarOpen && isMobile ? "mobile-sidebar" : ""} style={{ width: 230, background: "white", borderRight: "1.5px solid #EDE9FE", minHeight: "100vh", padding: "20px 12px", flexShrink: 0, boxShadow: "4px 0 20px rgba(124,58,237,0.06)", position: isMobile ? "fixed" : "relative", top: 0, left: 0, zIndex: 100, height: "100vh", overflowY: "auto", display: isMobile && !sidebarOpen ? "none" : "block" }}>
        ...
        {[{ icon: "📊", label: "Overview", tab: "overview" }, { icon: "🎪", label: "Kelola Event", tab: "events" }, { icon: "🎫", label: "Booking", tab: "bookings" }, { icon: "📈", label: "Laporan", tab: "report" }].map(item => (
          <div key={item.tab} className={`sidebar-item ${activeTab === item.tab ? "active" : ""}`} onClick={() => { setActiveTab(item.tab as any); if (isMobile) setSidebarOpen(false); }}>
            <span style={{ fontSize: 18, flexShrink: 0 }}>{item.icon}</span><span>{item.label}</span>
          </div>
        ))}
        <div style={{ marginTop: 12 }}>
          <div className="sidebar-item" onClick={handleLogout} style={{ color: "#DC2626" }} onMouseOver={e => (e.currentTarget.style.background = "#FEF2F2")} onMouseOut={e => (e.currentTarget.style.background = "transparent")}>
            <span style={{ fontSize: 18, flexShrink: 0 }}>🚪</span><span>Logout</span>
          </div>
        </div>
      ...
      <div style={{ flex: 1, minWidth: 0, paddingBottom: isMobile ? 72 : 0 }}>
        ...
        <div className="admin-navbar" style={{ background: "white", borderBottom: "1.5px solid #EDE9FE", padding: "0 24px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 50, boxShadow: "0 2px 12px rgba(124,58,237,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            {isMobile && <button onClick={e => { e.stopPropagation(); setSidebarOpen(!sidebarOpen); }} style={{ background: "#F5F3FF", border: "1.5px solid #DDD6FE", borderRadius: 10, width: 36, height: 36, cursor: "pointer", fontSize: 16, color: "#7C3AED", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>☰</button>}
            <div>
          ...
          <div ref={userMenuRef} style={{ position: "relative" }} onClick={e => e.stopPropagation()}>
            <div onClick={() => setShowUserMenu(!showUserMenu)} style={{ display: "flex", alignItems: "center", gap: 8, background: "#F5F3FF", border: "1.5px solid #DDD6FE", borderRadius: 12, padding: "7px 12px", cursor: "pointer" }}>
              <div style={{ width: 30, height: 30, borderRadius: "50%", background: "linear-gradient(135deg,#7C3AED,#F59E0B)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: 13, color: "white", flexShrink: 0 }}>{user?.name?.charAt(0).toUpperCase() || "A"}</div>
              <div className="hide-mobile"><div style={{ fontSize: 12, fontWeight: 800, color: "#1F2937" }}>{user?.name?.split(" ")[0] || "Admin"}</div><div style={{ fontSize: 9, color: "#7C3AED", fontWeight: 800 }}>⚡ Admin</div></div>
              <span style={{ color: "#9CA3AF", fontSize: 10 }}>{showUserMenu ? "▲" : "▼"}</span>
            </div>
            {showUserMenu && (
              <div className="dropdown-menu" style={{ position: "absolute", right: 0, top: 50, width: 180, background: "white", borderRadius: 16, padding: 8, zIndex: 200, boxShadow: "0 8px 32px rgba(124,58,237,0.18)", border: "1.5px solid #EDE9FE" }}>
                <div style={{ padding: "8px 12px", marginBottom: 4 }}><div style={{ fontSize: 13, fontWeight: 800, color: "#1F2937" }}>{user?.name}</div><div style={{ fontSize: 11, color: "#9CA3AF", fontWeight: 600 }}>{user?.email}</div></div>
                <div style={{ height: 1, background: "#EDE9FE", marginBottom: 4 }} />
                <button onClick={handleLogout} style={{ width: "100%", textAlign: "left", padding: "9px 12px", background: "none", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 700, color: "#DC2626", fontFamily: "'Nunito',sans-serif", borderRadius: 8, display: "flex", alignItems: "center", gap: 8 }} onMouseOver={e => (e.currentTarget.style.background = "#FEF2F2")} onMouseOut={e => (e.currentTarget.style.background = "none")}>🚪 <span>Logout</span></button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="admin-content" style={{ padding: "20px 24px" }}>
          <div className="tabs-scroll" style={{ display: "flex", gap: 8, marginBottom: 20 }}>
            {[{ id: "overview", label: "📊 Overview" }, { id: "events", label: "🎪 Event" }, { id: "bookings", label: "🎫 Booking" }, { id: "report", label: "📈 Laporan" }].map(t => (
          ...
          {activeTab === "overview" && (
            <div>
              <div style={{ marginBottom: 20 }}><h2 style={{ fontSize: 20, fontWeight: 900, color: "#1F2937", marginBottom: 2 }}>Halo, {user?.name}! 👋</h2><p style={{ color: "#9CA3AF", fontWeight: 600, fontSize: 13 }}>Ringkasan platform TiketIn hari ini</p></div>
              <div className="stats-grid" style={{ marginBottom: 20 }}>
                {[{ label: "Revenue", value: formatPrice(totalRevenue), icon: "💰", color: "#D97706", bg: "#FFFBEB" }, { label: "Booking", value: bookings.length, icon: "🎫", color: "#7C3AED", bg: "#F5F3FF" }, { label: "Sukses", value: paidBookings, icon: "✅", color: "#059669", bg: "#ECFDF5" }, { label: "Event", value: events.length, icon: "🎪", color: "#7C3AED", bg: "#EDE9FE" }].map((s, i) => (
                  <div key={i} className="stat-card" style={{ background: s.bg }}><div style={{ fontSize: 22, marginBottom: 8 }}>{s.icon}</div><div style={{ fontSize: i === 0 ? 13 : 26, fontWeight: 900, color: s.color, lineHeight: 1 }}>{s.value}</div><div style={{ fontSize: 11, color: "#6B7280", fontWeight: 700, marginTop: 4 }}>{s.label}</div></div>
                ))}
              </div>
                ...
                <div style={{ background: "white", borderRadius: 16, padding: 18, border: "1.5px solid #EDE9FE", boxShadow: "0 2px 12px rgba(124,58,237,0.06)" }}>
                  ...
                  {events.slice(0, 3).map((ev, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: i < 2 ? "1px solid #F3F4F6" : "none" }}>
                      <img src={ev.image} style={{ width: 40, height: 32, borderRadius: 8, objectFit: "cover", flexShrink: 0 }} alt={ev.title} onError={e => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?w=400&h=200&fit=crop"; }} />
                      <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 12, fontWeight: 800, color: "#1F2937", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ev.title}</div><div style={{ fontSize: 10, color: "#9CA3AF", fontWeight: 600 }}>{Math.round((ev.sold_tickets / ev.total_tickets) * 100)}% terjual</div></div>
                ...
                <div style={{ background: "white", borderRadius: 16, padding: 18, border: "1.5px solid #EDE9FE", boxShadow: "0 2px 12px rgba(124,58,237,0.06)" }}>
                  ...
                  {bookings.slice(0, 3).map((b, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: i < 2 ? "1px solid #F3F4F6" : "none" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 12, fontWeight: 800, color: "#1F2937", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{b.user.name}</div><div style={{ fontSize: 10, color: "#9CA3AF", fontWeight: 600 }}>{b.booking_code}</div></div>
                      <div style={{ textAlign: "right", flexShrink: 0 }}><div style={{ fontSize: 12, fontWeight: 900, color: "#7C3AED" }}>{formatPrice(toNum(b.total_price))}</div><span className={`status-${b.status}`}>{b.status === "paid" ? "✅" : "❌"}</span></div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {activeTab === "events" && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
                <div><h2 style={{ fontSize: 18, fontWeight: 900, color: "#1F2937", marginBottom: 2 }}>🎪 Kelola Event</h2><p style={{ color: "#9CA3AF", fontWeight: 600, fontSize: 12 }}>{events.length} event</p></div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  ...
                  <button className="btn-primary" onClick={() => { setShowModal(true); setEditEvent(null); setForm(emptyForm); setTickets([]); }} style={{ padding: "9px 18px", fontSize: 13 }}>+ Tambah</button>
                </div>
              </div>
                ...
                {filteredEvents.map((ev) => (
                  <div key={ev.id} style={{ background: "white", borderRadius: 16, border: "1.5px solid #EDE9FE", overflow: "hidden", boxShadow: "0 2px 12px rgba(124,58,237,0.06)" }}>
                    <div className="event-card-mobile" style={{ display: "flex" }}>
                      <img src={ev.image} style={{ width: 110, height: 90, objectFit: "cover", flexShrink: 0 }} alt={ev.title} onError={e => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?w=400&h=200&fit=crop"; }} />
                      <div style={{ flex: 1, padding: "12px 14px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
                          <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 900, color: "#1F2937", lineHeight: 1.3, marginBottom: 2 }}>{ev.title}</div><div style={{ fontSize: 11, color: "#9CA3AF", fontWeight: 600 }}>📍 {ev.venue} · {formatDate(ev.event_date)}</div></div>
                        ...
                        <div style={{ marginBottom: 10 }}>
                          ...
                          <div style={{ height: 5, background: "#EDE9FE", borderRadius: 99 }}><div style={{ height: "100%", width: `${Math.min((ev.sold_tickets / ev.total_tickets) * 100, 100)}%`, background: "linear-gradient(90deg,#7C3AED,#F59E0B)", borderRadius: 99 }} /></div>
                        </div>
                        <div style={{ display: "flex", gap: 8 }}>
                          {deleteConfirm === ev.id ? (
                            <><button className="btn-danger" onClick={() => handleDelete(ev.id)} style={{ padding: "5px 12px", fontSize: 11 }}>Hapus</button><button className="btn-ghost" onClick={() => setDeleteConfirm(null)} style={{ padding: "5px 12px", fontSize: 11 }}>Batal</button></>
                          ) : (
                            <><button className="btn-primary" onClick={() => { setEditEvent(ev); setForm({ title: ev.title, description: "", category: ev.category, location: ev.location, venue: ev.venue, event_date: ev.event_date.slice(0, 16), total_tickets: ev.total_tickets.toString(), image: ev.image }); setShowModal(true); fetchTickets(ev.id); }} style={{ padding: "5px 12px", fontSize: 11 }}>✏️ Edit & Tiket</button><button className="btn-danger" onClick={() => setDeleteConfirm(ev.id)} style={{ padding: "5px 12px", fontSize: 11 }}>🗑️</button></>
                          )}
                        </div>
                      </div>
                    </div>
          ...
          {activeTab === "bookings" && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
                <div><h2 style={{ fontSize: 18, fontWeight: 900, color: "#1F2937", marginBottom: 2 }}>🎫 Semua Booking</h2><p style={{ color: "#9CA3AF", fontWeight: 600, fontSize: 12 }}>{bookings.length} transaksi</p></div>
                ...
                {filteredBookings.map((b) => (
                  <div key={b.id} style={{ background: "white", borderRadius: 14, border: "1.5px solid #EDE9FE", padding: "14px 16px", boxShadow: "0 2px 12px rgba(124,58,237,0.06)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                      <div><div style={{ fontSize: 12, fontWeight: 800, color: "#7C3AED", marginBottom: 2 }}>{b.booking_code}</div><div style={{ fontSize: 11, color: "#9CA3AF", fontWeight: 600 }}>{formatDate(b.created_at)}</div></div>
                      <span className={`status-${b.status}`}>{b.status === "paid" ? "✅ Sukses" : "❌ Batal"}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div><div style={{ fontSize: 13, fontWeight: 800, color: "#1F2937" }}>{b.user.name}</div><div style={{ fontSize: 11, color: "#9CA3AF", fontWeight: 600 }}>{b.event.title.slice(0, 20)}... · {b.ticket.name} ×{b.quantity}</div></div>
          ...
          {activeTab === "report" && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, flexWrap: "wrap", gap: 10 }}>
                <div><h2 style={{ fontSize: 18, fontWeight: 900, color: "#1F2937", marginBottom: 2 }}>📈 Laporan & Analitik</h2><p style={{ color: "#9CA3AF", fontWeight: 600, fontSize: 12 }}>Performa penjualan TiketIn</p></div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button className="btn-download" onClick={downloadCSV} style={{ padding: "9px 16px", fontSize: 12 }}>📥 Download CSV</button>
                  <button className="btn-ghost" onClick={downloadSummary} style={{ padding: "9px 16px", fontSize: 12 }}>📄 Ringkasan</button>
                </div>
              </div>
                  ...
                  <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 130 }}>
                    {monthlyData.map((d, i) => (
                      <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, height: "100%" }}>
                        <div style={{ width: "100%", flex: 1, display: "flex", alignItems: "flex-end" }}><div style={{ width: "100%", height: animateBars ? `${(d.revenue / maxRevenue) * 100}%` : "0%", background: i === monthlyData.length - 1 ? "linear-gradient(180deg,#7C3AED,#9333EA)" : "linear-gradient(180deg,#DDD6FE,#EDE9FE)", borderRadius: "6px 6px 0 0", transition: `height 0.8s cubic-bezier(0.34,1.56,0.64,1) ${i * 0.1}s` }} /></div>
                        <span style={{ fontSize: 10, fontWeight: 700, color: i === monthlyData.length - 1 ? "#7C3AED" : "#9CA3AF" }}>{d.month}</span>
                      </div>
                    ))}
                  </div>
                  <div style={{ marginTop: 10, padding: "8px 12px", background: "#F5F3FF", borderRadius: 10, fontSize: 11, fontWeight: 800, color: "#7C3AED" }}>Total: {formatPrice(monthlyData.reduce((s, d) => s + d.revenue, 0))}</div>
                </div>
                <div style={{ background: "white", borderRadius: 16, padding: 18, border: "1.5px solid #EDE9FE", boxShadow: "0 2px 12px rgba(124,58,237,0.06)" }}>
                  <h3 style={{ fontSize: 14, fontWeight: 900, color: "#1F2937", marginBottom: 14 }}>📊 Ringkasan</h3>
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    {[{ label: "Total Revenue", value: formatPrice(totalRevenue), color: "#D97706" }, { label: "Booking Berhasil", value: `${paidBookings} transaksi`, color: "#059669" }, { label: "Booking Batal", value: `${bookings.length - paidBookings} transaksi`, color: "#DC2626" }, { label: "Event Aktif", value: `${events.length} event`, color: "#7C3AED" }, { label: "Konversi", value: `${bookings.length > 0 ? Math.round((paidBookings / bookings.length) * 100) : 0}%`, color: "#7C3AED" }].map((item, i) => (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {[...events].sort((a, b) => b.sold_tickets - a.sold_tickets).map((ev, i) => (
                    <div key={ev.id} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <img src={ev.image} style={{ width: 44, height: 32, borderRadius: 8, objectFit: "cover", flexShrink: 0 }} alt={ev.title} onError={e => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?w=400&h=200&fit=crop"; }} />
                      <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 800, color: "#1F2937", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ev.title}</div><div style={{ height: 5, background: "#EDE9FE", borderRadius: 99, marginTop: 5 }}><div style={{ height: "100%", width: `${(ev.sold_tickets / ev.total_tickets) * 100}%`, background: "linear-gradient(90deg,#7C3AED,#F59E0B)", borderRadius: 99 }} /></div></div>
                      <div style={{ textAlign: "right", flexShrink: 0 }}><div style={{ fontSize: 13, fontWeight: 900, color: "#7C3AED" }}>{ev.sold_tickets.toLocaleString()}</div><div style={{ fontSize: 10, color: "#9CA3AF", fontWeight: 600 }}>terjual</div></div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="bottom-nav" style={{ justifyContent: "space-around", alignItems: "center" }}>
        {[{ icon: "📊", label: "Overview", tab: "overview" }, { icon: "🎪", label: "Event", tab: "events" }, { icon: "🎫", label: "Booking", tab: "bookings" }, { icon: "📈", label: "Laporan", tab: "report" }].map((item) => (
      ...
      {/* MODAL TAMBAH/EDIT EVENT + TIKET */}
      {showModal && (
        <div className="modal-overlay" onClick={() => { setShowModal(false); setTickets([]); }}>
          <div className="modal-box" onClick={e => e.stopPropagation()} style={{ background: "white", border: "1.5px solid #DDD6FE", borderRadius: 24, padding: "24px 20px", maxWidth: 560, width: "100%", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 32px 80px rgba(124,58,237,0.2)" }}>

            {/* Header modal */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ fontSize: 18, fontWeight: 900, color: "#1F2937" }}>{editEvent ? "✏️ Edit Event" : "🎪 Tambah Event"}</h3>
              <button onClick={() => { setShowModal(false); setTickets([]); }} style={{ width: 30, height: 30, borderRadius: "50%", background: "#F5F3FF", border: "1.5px solid #DDD6FE", cursor: "pointer", fontSize: 14, color: "#7C3AED", display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
            </div>

            {/* Form event */}
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {[{ label: "Judul Event", key: "title", placeholder: "Nama event" }, { label: "Deskripsi", key: "description", placeholder: "Deskripsi event", textarea: true }, { label: "Lokasi (Kota)", key: "location", placeholder: "Jakarta, Bali, dll" }, { label: "Venue", key: "venue", placeholder: "Nama venue" }, { label: "Total Tiket", key: "total_tickets", placeholder: "Jumlah tiket", type: "number" }, { label: "URL Gambar", key: "image", placeholder: "https://..." }].map(field => (
                <div key={field.key}>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                ...
                <button className="btn-ghost" onClick={() => { setShowModal(false); setTickets([]); }} style={{ padding: "12px 20px", fontSize: 14 }}>Batal</button>
              </div>

              {/* ── SECTION TIKET (hanya saat edit) ── */}
              {editEvent && (
                <div>
                  <div style={{ height: 1, background: "#EDE9FE", margin: "8px 0 16px" }} />
                  <h4 style={{ fontSize: 15, fontWeight: 900, color: "#1F2937", marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
                    🎫 Kelola Tiket
                    <span style={{ fontSize: 11, color: "#9CA3AF", fontWeight: 600 }}>({tickets.length} tiket)</span>
                  </h4>
                  ...
                  {tickets.length > 0 ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
                      {tickets.map((t: any) => (
                        <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#F5F3FF", borderRadius: 12, padding: "12px 14px", border: "1.5px solid #EDE9FE" }}>
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 900, color: "#1F2937", marginBottom: 2 }}>{t.name}</div>
                            <div style={{ fontSize: 11, color: "#6B7280", fontWeight: 600 }}>
                              {formatPrice(Number(t.price))} · {Number(t.quota).toLocaleString()} tiket
                              {t.description && ` · ${t.description}`}
                            </div>
                          </div>
                          <button onClick={() => handleDeleteTicket(t.id)} style={{ background: "#FEF2F2", border: "1.5px solid #FECACA", borderRadius: 8, padding: "5px 10px", fontSize: 12, fontWeight: 700, color: "#DC2626", cursor: "pointer", flexShrink: 0 }}>🗑️</button>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div style={{ textAlign: "center", padding: "16px 0", color: "#9CA3AF", fontSize: 13, fontWeight: 600, marginBottom: 12 }}>
                      Belum ada tiket — tambah tiket di bawah!
                    </div>
                  )}

                  {/* Form tambah tiket */}
                  <TicketForm onAdd={handleAddTicket} loading={ticketLoading} />
                </div>
              )}

              {/* Info jika event baru */}
              {!editEvent && (
                <div style={{ background: "#FFFBEB", borderRadius: 12, padding: "12px 14px", border: "1px solid #FDE68A", fontSize: 12, color: "#D97706", fontWeight: 600 }}>
                  💡 Setelah event dibuat, klik Edit untuk menambahkan tiket (Festival, VIP, VVIP, dll)
                </div>
              )}
            </div>
          </div>
